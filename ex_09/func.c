 ...
 * @file func.c
 * @date 2017/10/30
 * @author Yuta Kobiyama (m5191140@u-aizu.ac.jp), Masato Hashimoto
 * (m5201129@u-aizu.ac.jp)
...
void print_car_speed_control_fuzzy(void) {
    ...
    FuzzyRule rules[] = {
        ...
        [0] = {.function_count = 3,
               .membership_funcs = {membership_speed_normal,
                                    membership_curve_steep,
                                    membership_brake_weak},
               .function_arguments = {&velocity, &radius, &deceleration}},

        // Define fuzzy rule 2.
        //  If (v = A little fast) and (r = Steep curve), Then (b = Weak)
        [1] = {.function_count = 3,
               .membership_funcs = {membership_speed_bit_fast,
                                    membership_curve_steep,
                                    membership_brake_weak},
               .function_arguments = {&velocity, &radius, &deceleration}},
        ...
        [2] = {.function_count = 3,
               .membership_funcs = {membership_speed_fast,
        ...
        [3] = {
            .function_count = 3,
            .membership_funcs = {membership_speed_fast, membership_curve_steep,
    ...
    for (velocity = 20.0; velocity <= 80.0; velocity += delta) {
        ...
        for (radius = 75.0; radius <= 225.0; radius += delta) {
            ...
            for (deceleration = 0.0; deceleration <= 40.0;
                 deceleration += delta) {
 ...
 */
static double base_membership(double left, double center, double right,
                              double x) {
    if ((x <= left) || (right <= x)) {
        // Out of the triangle.
        return 0.0;
